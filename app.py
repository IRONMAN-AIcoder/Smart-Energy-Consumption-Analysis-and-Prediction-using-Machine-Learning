...
df_feat["date"] = df_feat["time"].dt.date
df_feat["week"] = df_feat["time"].dt.to_period("W").astype(str)
df_feat["month"] = df_feat["time"].dt.to_period("M").astype(str)
...
def generate_charts():
    ...
    hourly = df_feat.groupby("hour")["House overall [kW]"].mean()
    ...
    daily = df_feat.groupby("date")["House overall [kW]"].sum()
    ...
    weekly = df_feat.groupby("week")["House overall [kW]"].sum()
    ...
    monthly = df_feat.groupby("month")["House overall [kW]"].sum()
    ...
    device_usage = df_feat.groupby("device")["power_kW"].sum()
...
@app.route("/devices")
def get_devices():
    encoded_ids = sorted(df_feat["device"].unique())
...
def dashboard():
    generate_charts()
    tips = smart_suggestions(df_feat)
